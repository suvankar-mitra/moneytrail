...
public class AccountControllerIT {
    ...
    public void setup() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON).content(validJsonRequest))
                .andReturn();

        var authResponse = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), AuthResponse.class);
        ...
        String validJson1 = """
                ...
                """;
        mvcResult = mockMvc.perform(post("/api/v1/accounts")
                        .header("Authorization", "Bearer " + user1Token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(validJson1))
                .andReturn();
        accountResponseList.add(objectMapper.readValue(mvcResult.getResponse().getContentAsString(),
        ...
        mvcResult = mockMvc.perform(post("/api/v1/auth/login")
                        .contentType(MediaType.APPLICATION_JSON).content(validJsonRequest))
                .andReturn();

        authResponse = objectMapper.readValue(mvcResult.getResponse().getContentAsString(), AuthResponse.class);
    ...
    public void getAccounts_shouldReturn200AndListOfAccounts_whenValidUser() throws Exception {
        ...
        assertThat(listOfObjects.stream().map(AccountResponse::getName).toList())
                .containsExactlyInAnyOrderElementsOf(accountResponseList.stream().map(AccountResponse::getName).toList());
        assertThat(listOfObjects.stream().map(AccountResponse::getAccountId).toList())
                .containsExactlyInAnyOrderElementsOf(accountResponseList.stream().map(AccountResponse::getAccountId).toList());
    }
